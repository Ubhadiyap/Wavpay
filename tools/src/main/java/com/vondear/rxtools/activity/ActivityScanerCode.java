...
import com.vondear.rxtools.callback.IToolsCallback;
import com.vondear.rxtools.callback.ToolsCallbackManager;
import com.vondear.rxtools.callback.ToolsCallbackType;
...
public class ActivityScanerCode extends ActivityBase implements View.OnClickListener{
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        initView();//界面控件初始化
        initScanAnimation();//扫描动画初始化
        CameraManager.init(mContext);//初始化 CameraManager
        hasSurface = false;
        inactivityTimer = new InactivityTimer(this);
        ToolsCallbackManager.getInstance()
                .addCallback(ToolsCallbackType.FINISH, new IToolsCallback() {
                    @Override
    ...
    private void initView() {
        mContainer = $(R.id.capture_containter);
        mCropLayout = $(R.id.capture_crop_layout);
        ...
        $(R.id.top_mask).setOnClickListener(this);
        $(R.id.top_back).setOnClickListener(this);
        $(R.id.tv_album).setOnClickListener(this);
    }

    private void initScanAnimation() {
        ImageView mQrLineView = $(R.id.capture_scan_line);
            ...
            try {
                ...
                if (rawResult != null) {
                    if (mScanerListener == null) {
                        IToolsCallback<Result> callback = ToolsCallbackManager.getInstance().getCallback(ToolsCallbackType.QR_CODE);
                        callback.executeCallback(rawResult);
                    } else {
