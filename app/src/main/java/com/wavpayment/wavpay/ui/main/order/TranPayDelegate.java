...
public class TranPayDelegate extends SpongeDelegate {
    ...
    public Object setLayout() {
        return R.layout.delegate_tran_pay;
    }
                    ...
                    .execute(new StringCallback() {
                        ...
                        public void onSuccess(Response<String> response) {
                            ...
                            if (code == 200) {
                                ...
                                final String collectWays = bill.getString("collectWays");
                                ...
                                final int tradeType = bill.getIntValue("tradeType");
                                ((TextView) $(R.id.tv_time)).setText(tradeTime);
                                ((TextView) $(R.id.tv_amount)).setText("RM "+String.valueOf(collectAmount));
                                ((TextView) $(R.id.tv_name)).setText(String.valueOf(orderId));
                                if (tradeType == 5) {
                                    final String payMemo = bill.getString("payMemo");
                                    ((TextView) $(R.id.tv_type)).setText(payMemo);
                                } else {
                                    if (collectWays != null) {
                                        ((TextView) $(R.id.tv_type)).setText(collectWays);
                                    } else {
                                        final int ct = bill.getIntValue("collectAccountType");
                                        if (ct==1) {
                                            final String name = bill.getString("collectAccount");
                                            ((TextView) $(R.id.tv_type)).setText(name);
                                        }else if (ct==2){
                                            final String name = bill.getString("collUserName");
                                            ((TextView) $(R.id.tv_type)).setText(name);
                                        }
                                    }
                                }
